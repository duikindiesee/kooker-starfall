...
namespace Starfall.Refuge
{
 public sealed class RefugeRuntime:MonoBehaviour
 {
  ...
  float yaw,pitch,fall;bool paused;ZoneExposure exposure;public ZoneWeather Local;string output;
  ...
  void FixedUpdate(){if(paused)return;Clock.Step();var w=Sample(Hearth+Vector3.up*.5f);Fire.Step(w.Rain01,w.WindSpeed,SafeFire());Local=Sample(Body.transform.position+Vector3.up);exposure.Step(Local,false);}
  ...
  public bool Rest(){if(Vector3.Distance(Body.transform.position,Bed)>2)return false;Resting=!Resting;Notice=Resting?"Resting on the mat — dreams and memory are planned":"Awake";return true;}
  void Update()
  {
   frames.Add(Time.unscaledDeltaTime*1000);Flame.SetActive(Fire.Burning);FireLight.enabled=Fire.Burning;FireLight.intensity=Fire.Burning?2.5f+.15f*Mathf.Sin(Fire.Tick*.17f):0;
  ...
  void OnGUI(){GUI.Box(new Rect(12,12,690,130),"STARFALL — FIRST REFUGE 0.0.8-refuge.1");GUI.Label(new Rect(25,38,670,24),"WASD walk · RMB look · E fire · T stored fuel · R rest/wake · P pause");GUI.Label(new Rect(25,62,670,24),$"{Fire.Reason} · fuel {Fire.FuelTicks/50f:F0}s · stored logs {Fire.ReserveLogs} · {(Resting?"RESTING":"AWAKE")}");GUI.Label(new Rect(25,86,670,24),$"{Clock.Sample.target}: local wind {Local.WindSpeed:F1}m/s · rain {Local.Rain01:P0} · feels {exposure.ApparentC:F1}C");GUI.Label(new Rect(25,110,670,24),Notice);if(Resting){GUI.Box(new Rect(Screen.width/2-190,Screen.height-100,380,70),"Resting in the first refuge\nR / Escape to wake · dream integration planned");}}
  ...
  IEnumerator Accept()
  {
   ...
   yield return Route(new Vector3(-7,1.8f,0));yield return Route(new Vector3(-10.5f,1.8f,-.7f));Check("rest interaction",Rest());yield return Capture("04-rest",Bed+new Vector3(.8f,.45f,0),Hearth+Vector3.up*.5f);Check("rest state visible",Resting);Rest();Check("wake releases movement",!Resting);
