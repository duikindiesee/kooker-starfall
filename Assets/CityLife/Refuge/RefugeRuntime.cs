...
namespace Starfall.Refuge
{
 public sealed class RefugeRuntime:MonoBehaviour
 {
  ...
  void Update()
  {
   ...
   if(!Automated&&Application.isFocused){var k=Keyboard.current;var m=Mouse.current;if(k!=null){if(k.pKey.wasPressedThisFrame)paused=!paused;if(k.eKey.wasPressedThisFrame)ToggleFire();if(k.rKey.wasPressedThisFrame)Rest();if(k.tKey.wasPressedThisFrame&&Vector3.Distance(Body.transform.position,Storage)<2)Notice=TransferLog()?"Moved one stored log to hearth":"Storage empty or hearth full";if(k.escapeKey.wasPressedThisFrame)Resting=false;Vector3 d=new Vector3((k.dKey.isPressed?1:0)-(k.aKey.isPressed?1:0),0,(k.wKey.isPressed?1:0)-(k.sKey.isPressed?1:0));Move(Quaternion.Euler(0,yaw,0)*d,Mathf.Min(Time.deltaTime,.05f));}if(m!=null&&m.rightButton.isPressed){var delta=m.delta.ReadValue();yaw+=delta.x*.12f;pitch=Mathf.Clamp(pitch-delta.y*.12f,-65,65);}}
