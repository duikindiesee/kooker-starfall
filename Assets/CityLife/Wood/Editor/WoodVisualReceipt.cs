...
namespace CityLife.Wood.Editor {
 public static class WoodVisualReceipt {
  ...
  public static void Run(){
   ...
   try{
    ...
    var texture=CityLife.Wood.FallenWoodGenerator.GenerateProceduralBarkTexture(4217,256,256);
    var woodMat=Material(Color.white);woodMat.SetTexture("_BaseMap",texture);
    report.texture="Existing author-produced GenerateProceduralBarkTexture(4217,256,256) on URP/Lit inspection material; end-grain/vertex palette integration not asserted";
    var profiles=new[]{CityLife.Wood.FallenWoodProfile.CreateBranchPreset(),CityLife.Wood.FallenWoodProfile.CreateLogProfile(),CityLife.Wood.FallenWoodProfile.CreateKindlingPreset()};
    report.stones=new Entry[3];
    for(int i=0;i<3;i++){
     var mesh=CityLife.Wood.FallenWoodGenerator.GenerateMesh(profiles[i],4217u);
     var go=new GameObject(profiles[i].profileName);go.AddComponent<MeshFilter>().sharedMesh=mesh;go.AddComponent<MeshRenderer>().sharedMaterial=woodMat;go.transform.rotation=Quaternion.Euler(0,90,0);go.transform.position=new Vector3(-.25f,-mesh.bounds.min.y,(i-1)*.4f);
     report.stones[i]=new Entry{shape=profiles[i].profileName,size=mesh.bounds.size,vertices=mesh.vertexCount,triangles=mesh.triangles.Length/3};
    }
    ...
    string generated="Assets/CityLife/Wood/Editor/GeneratedVisualEvidence39";Directory.CreateDirectory(generated);AssetDatabase.Refresh();
  ...
  static void Tick(){if(++frames<30)return;EditorApplication.update-=Tick;try{
    actor.SetActive(false);Shot("wood-closeup.png",new Vector3(1.1f,1.0f,-1.9f),new Vector3(0,.06f,0));
    actor.SetActive(true);Shot("wood-inhabitant-scale.png",new Vector3(2.6f,1.8f,-5.0f),new Vector3(.35f,1.0f,.1f));
    report.images=new[]{"wood-closeup.png","wood-inhabitant-scale.png"};report.status="RENDERED";Write();EditorApplication.Exit(0);
   }catch(Exception e){Fail(e);}}
