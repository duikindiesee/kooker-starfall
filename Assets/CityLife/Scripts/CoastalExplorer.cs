...
namespace CityLife.World
{
    ...
    /// </summary>
    public sealed class CoastalExplorer : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            if (Camera == null) { Debug.LogError("COASTAL_PLAYER: no camera assigned."); enabled = false; return; }
        ...
        private void Update()
        {
            if (CoastalSmoke.Requested || !ready) return;
        ...
        private void Move(Vector3 input, bool fast, float seconds)
        {
            ...
            else
            {
                ...
                for (int i = 0; i < steps; i++)
                {
                    ...
                    if (!TryGround(candidate, out RaycastHit ground) || (ground.normal.y < MaximumSlopeCosine || ground.point.y < CoastalWater.Level + .15f)) break;
        ...
        {
            position.x = Mathf.Clamp(position.x, CoastalTerrain.MinX + 1f, CoastalTerrain.MaxX - 1f);
            position.z = Mathf.Clamp(position.z, CoastalTerrain.MinZ + 1f, CoastalTerrain.MaxZ - 1f);
            return position;
        }
        ...
        private void ToggleMode()
        {
            if (flying)
            {
                ...
                if (!TryGround(position, out RaycastHit ground) || (ground.normal.y < MaximumSlopeCosine || ground.point.y < CoastalWater.Level + .15f))
                {
        ...
        {
            if (CoastalSmoke.Requested) return;
            if (!looking) return;
            looking = false;
        ...
        {
            if (CoastalSmoke.Requested || focused) return;
            ReleasePointer();
        ...
        {
            Debug.Log("COASTAL_PLAYER_INPUT " + JsonUtility.ToJson(new InputRecord
            {
                ...
                automatic = CoastalSmoke.Requested
            }));
        }
        ...
        private void CaptureRequested()
        {
            ...
            try
            {
                ...
                string file = "coastal-player-" + DateTime.UtcNow.ToString("yyyyMMdd-HHmmss-fff", CultureInfo.InvariantCulture) + ".png";
        ...
        private void OnGUI()
        {
            ...
            GUI.Label(new Rect(24, 48, panelWidth - 24, 35), "Coastal slice · rocky bank, river and sea · dry-shore walking / inspection flight", bodyStyle);
        ...
        {
            if (!CoastalSmoke.Requested || !ready) throw new InvalidOperationException("Smoke hook unavailable.");
