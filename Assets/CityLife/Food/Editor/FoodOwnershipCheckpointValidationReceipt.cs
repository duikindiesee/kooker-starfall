...
namespace CityLife.Items.Editor {
 public static class FoodOwnershipCheckpointValidationReceipt {
  ...
  public static void Run() {
   ...
   try { receipt.checks=Starfall.Food.FoodOwnershipCheckpointChecks.Run(Path.Combine(args[index+1],"fixtures")).ToArray(); receipt.status="PASS"; code=0; }
   ...
   Debug.Log("FOOD_CHECKPOINT_SUITE "+receipt.status+" named checks="+(receipt.checks==null?0:receipt.checks.Length));
