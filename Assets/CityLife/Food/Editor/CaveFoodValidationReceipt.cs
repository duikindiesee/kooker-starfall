...
namespace CityLife.Items.Editor {
 public static class CaveFoodValidationReceipt {
  ...
  public static void Run() {
   ...
   try { receipt.checks=Starfall.Food.CaveFoodMemoryChecks.Run(Path.Combine(args[index+1],"fixtures")).ToArray(); receipt.status="PASS"; code=0; }
   ...
   Debug.Log("CAVE_FOOD_SUITE "+receipt.status+" named checks="+(receipt.checks==null?0:receipt.checks.Length));
