...
namespace CityLife.World.Editor
{
    ...
    public static class CoastalPlayerBuild
    {
        public const string R19Version="0.0.3-coastal.1";
        ...
        public static void Build(Camera camera,GameObject ground,Dictionary<string,string> shaderSources,string evidenceDirectory,bool frozenR19=false,string sourceCommit="",string componentHash="")
        {
            ...
            foreach (MeshCollider existingCollider in Object.FindObjectsByType<MeshCollider>(FindObjectsInactive.Include,FindObjectsSortMode.None)) existingCollider.sharedMesh=(Mesh)Persist(existingCollider.sharedMesh);
            ...
            try
            {
                ...
                evidence.woodColliders=woodColliders;evidence.rockColliders=84;
                if(frozenR19)
                {
                    evidence.treeReview="R19";evidence.frozenTreeCommit=FrozenTreeCommit;evidence.componentMeshSha256=componentHash;
                    evidence.scope="Separate coastal player, frozen PH02 tree, terrain and 84 rock colliders. Dry-shore walking and inspection flight. No swimming or saved-world integration. Native acceptance recorded separately.";
                }
