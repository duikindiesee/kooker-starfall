...
function RequireBoolean($value) {
    if ($value -isnot [bool]) { throw 'Evidence flags must be JSON booleans, not truthy strings or numbers.' }
}
function RequireNumber($value) {
    if (($value -isnot [long] -and $value -isnot [int] -and $value -isnot [double] -and $value -isnot [decimal]) -or ![double]::IsFinite([double]$value)) { throw 'Finite JSON numeric measurement required.' }
}
...
RequireBoolean $p.normalFlagsVerified
foreach($name in @('npcSurvivalRuntime','npcSurvivalDeathAcceptance','integratedSmoke','npcSmoke','npcRealProbe')) { RequireBoolean $p.flags.$name }
RequireNumber $normal.durationSecondsAtFinalLiveSample
...
foreach($event in $events) { RequireNumber $event.tick }
...
RequireBoolean $dp.flags.npcSurvivalDeathAcceptance
...
RequireBoolean $d.geometryPreserved
...
foreach($cause in @('prolonged-dehydration','prolonged-starvation')) {
    ...
    $c=$cases[0]
    foreach($name in @('realPhysiologyDeath','safeRefugeReturn','scopedReload','worldAndActorPreserved','noInventedKnowledge')) { RequireBoolean $c.$name }
    RequireNumber $c.incarnationBefore
    RequireNumber $c.incarnationAfter
