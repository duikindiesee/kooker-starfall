...
function InspectKeep([string]$manifestPath,[string]$runtimePath){
...
$fallback=InspectKeep $FallbackManifest $FallbackRuntime
$candidate=InspectKeep $CandidateManifest $CandidateRuntime
...
$receipt=[ordered]@{
    ...
    fallback=$fallback;candidate=$candidate;targets=$targets;skipped=$skipped;
    targetCount=@($targets).Count;targetBytes=($targets | Measure-Object bytes -Sum).Sum;
    freeBefore=(Get-PSDrive C).Free;freeAfter=$null;
...
try{
    ...
    $null=InspectKeep $FallbackManifest $FallbackRuntime
    $null=InspectKeep $CandidateManifest $CandidateRuntime
    $receipt.status='RETAINED_FALLBACK_AND_TESTED_CANDIDATE_NOT_RELEASE'
