...
### Protocol v3 candidate — compiled proof pending

Post-reboot measurements showed that LM Studio's grammar for a top-level JSON string repeatedly produced truncated invalid output with `finish_reason=length`. Three warm, laptop-local E4B requests using the exact v2 payload took 7174, 5134 and 6132 ms and returned no admissible thought. This was a protocol incompatibility as well as a latency failure, so the candidate source does not hide it by increasing only the timeout.

Protocol v3 removes that incompatible response grammar and accepts only two printable plain-text tokens: `Delivered` plus the exact item later rechecked against the verified event and current registry. It remains interpretation-only, makes one attempt, and cannot propose or execute actions. The existing 1500 ms deadline remains unchanged while the protocol is measured independently. Pause, possession, stopped autonomy, stale identity, stale delivery state, timeout, extra text and malformed output still discard the response. This design is source-only until a new compiled player proves a genuine response, current-state admission and safe fallback.
