...
Bring `Assets/CityLife/Food/FoodModel.cs`, `EdenEcology.cs` and `FoodPhysiology.cs` into a reviewed integration branch, plus their Unity metadata. `FoodWorld` and `FoodBuild` are the separate fixture and must not replace the combined-world bootstrap. `FoodChecks` can run in that branch as a contract suite.
...
The physiology checks cover fasting, activity, fat use, sustained surplus, sheltered recovery, delayed protein effects and eventual starvation/dehydration. They do not alone establish rendered/input acceptance. The compiled mortality scenario explicitly starts at a synthetic final starvation boundary, then verifies actual death/save/reload/return/recovery, insight and a subsequent meal using that lesson; it must never be described as hours of natural play.
