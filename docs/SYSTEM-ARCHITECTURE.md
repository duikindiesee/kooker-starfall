...
| Optional local LLM adapter | Implemented and tested for bounded/fake/offline behavior | Optional Unity adapter; disabled by default. Endpoint/model are explicit and operator-managed. Normal deadline is 1500 ms. | [Provider/planner setup and limits](HYBRID-NPC.md), [48 focused diagnostic checks](../evidence/verified/hybrid-diagnostic-validation.json) | Full action-proposal acceptance remains unverified. One reflection-only thought passed in isolated Editor Play Mode (see below). No model is bundled or automatically loaded. |
...
## Isolated living-memory candidate: Editor thought proven, standalone blocked

[One living-memory thought](LIVING-MEMORY-THOUGHT.md) records a successful focused Unity Editor Play Mode run at source `fda786a434b6618b8d405e731b7262c7f140a61d`: real delivery, three scoped receipts, retrieval/restart, strict live admission and visible model thought in 991 ms against the unchanged 1500 ms deadline. The already-loaded linked E4B generated "Delivered Amber" in one request. Reflection-only output is a strict JSON string; event/world/actor correlation and action authority remain in Unity. Local evidence is `evidence/local/living-memory/run-20260914-editor-two-word/`. The reviewed scoped HTTP/SQLite interface is unchanged. Standalone preview.2 was blocked by Windows signing policy; Editor success does not establish standalone, keyboard-control, mainline or release acceptance.

```mermaid
flowchart LR
  A[Actual Editor Play Mode receipt] --> E[New explicit JSONL export]
  ...
  M --> G[Strict parse and live admission - 991 ms]
